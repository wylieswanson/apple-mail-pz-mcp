...
| # | Option | Trade-off |
|---|--------|-----------|
| a | User-supplied Keychain item under a known service name (e.g. `apple-mail-pz-mcp.<account>`) populated via a setup script | Simplest; works with app passwords across all providers; requires one-time user setup |
